...
| Waitlist email, optional role, optional value range, source page | One launch email and list administration | Consent-gated waitlist form | Consent where applicable | Telegram private chat via Bot API | Manual rule: verified withdrawal; otherwise within 30 days after launch email; target no later than 365 days after submission | Owner deletes matching Telegram message after verified request; account timer still unverified | Access/correction/deletion request through `hello@aipolicyfile.com` | Form values explicitly excluded | Route avoids application logging; provider network/security logs may exist | Implemented transport; automated deletion not yet verified |
...
- Before launch, verify Telegram auto-delete and purge legacy waitlist messages; verify GA4 retention and Enhanced Measurement; verify the receiving mailbox provider, MFA, and retention.
