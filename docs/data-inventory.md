...
| Contact email address, headers, and sender-chosen message | Respond to contact, privacy, correction, accessibility, security, order, or refund requests | `hello@aipolicyfile.com` | Consent, purchase contract, legitimate interests, or legal obligation depending on request | Namecheap forwarding to an owner-controlled Google Gmail mailbox | Retain only as needed for the request and legal/security recordkeeping; no automated account-side deletion rule is verified | Delete in the receiving mailbox, subject to provider backup, security, and legal limitations | Sender may request access, correction, or deletion where applicable | Prohibited | Namecheap and Google maintain delivery/security logs | **Inbound verified 2026-08-29:** external test `APF-20260829-01` reached Inbox and the `AI Policy File Support` filter applied its dedicated label. Outbound customer-facing identity, MFA/recovery, retention, deletion/export, and incident workflow remain launch gates |
...
- Telegram auto-delete is verified for new messages. Before launch, review pre-timer messages against the same withdrawal, post-launch, and 365-day limits; reverify GA4 after any account, property, or stream change; verify the support reply identity; and verify receiving-mailbox MFA, recovery, retention, deletion/export, and incident handling.
